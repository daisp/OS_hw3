...
bool testProduce() {
    ...
    for (list<Product>::iterator iterator = avProds.begin(), end = avProds.end();
         iterator != end; ++iterator) {
        ASSERT_TEST((*iterator).getId() == temp_ids2[i] &&
                    (*iterator).getValue() == temp_values2[i]);
        i++;
    }
...
bool testStandardRun() {
    ...
    ASSERT_TEST((*(avProds.begin())).getId() == 1 &&
                (*(avProds.begin())).getValue() == 3);
    ...
    ASSERT_TEST(((*(stolProds.begin())).first).getId() == 1 &&
                ((*(stolProds.begin())).first).getValue() == 3 &&
                (*(stolProds.begin())).second == 842);
    return true;
}
...
bool testOpenAndClose() {
    ...
    factory.startCompanyBuyer(7, 4,
                              11); // company that should run 3rd after opening, only when more products will be added
    ...
    factory.startCompanyBuyer(1, 1,
                              12); // company that should run 2nd after opening
    ...
    ASSERT_TEST(factory.finishCompanyBuyer(12) ==
                0); // company 11 can't run, so company 12 should run

    factory.startThief(1, 102);
    ASSERT_TEST(factory.finishThief(102) ==
                1); // a thief will take the product, after company 12 that has already finished

    stolProds = factory.listStolenProducts();
    ASSERT_TEST(stolProds.size() == 3);
    list<pair<Product, int>>::iterator iterator = stolProds.begin();
    ASSERT_TEST(((*(iterator)).first).getId() == 4 &&
                ((*(iterator)).first).getValue() == 11 &&
                (*(iterator)).second == 101);
    ++iterator;
    ASSERT_TEST(((*(iterator)).first).getId() == 5 &&
                ((*(iterator)).first).getValue() == 17 &&
                (*(iterator)).second == 101);
    ++iterator;
    ASSERT_TEST(((*(iterator)).first).getId() == 7 &&
                ((*(iterator)).first).getValue() == 1 &&
                (*(iterator)).second == 102);
